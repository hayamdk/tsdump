...
const DWORD PutSymbolsChar(TSDCHAR *lpszDst, const WORD wCode)
{
	...
	static const TSDCHAR *aszSymbolsTable2[] =
		{
			...
			_T("⒐"),		_T("氏"),		_T("副"),		_T("元"),		_T("故"),		_T("前"),		_T("新"),		_T("0,"),			// 0x7C39 - 0x7C40	92/25 - 92/32
			_T("1,"),		_T("2,"),		_T("3,"),		_T("4,"),		_T("5,"),		_T("6,"),		_T("7,"),		_T("8,"),			// 0x7C41 - 0x7C48	92/33 - 92/40
			_T("9,"),		_T("㈳"),		_T("㈶"),		_T("㈲"),		_T("㈱"),		_T("㈹"),		_T("(問)"),		_T("▶"),			// 0x7C49 - 0x7C50	92/41 - 92/48
			_T("◀"),		_T("〖"),		_T("〗"),		_T("⟐"),		_T("²"),		_T("³"),		_T("(CD)"),		_T("(vn)"),			// 0x7C51 - 0x7C58	92/49 - 92/56
	...
	static const TSDCHAR *aszSymbolsTable3[] =
		{
			...
			_T("［左］"),	_T("［中］"),	_T("［右］"),	_T("［指］"),	_T("［走］"),	_T("［打］"),	_T("ℓ"),		_T("㎏"),			// 0x7D41 - 0x7D48	93/33 - 93/40
			_T("㎐"),		_T("㏊"),		_T("㎞"),		_T("㎢"),		_T("㍱"),		_T("・"),		_T("・"),		_T("½"),			// 0x7D49 - 0x7D50	93/41 - 93/48
			_T("⅟₃"),		_T("⅓"),		_T("⅔"),		_T("¼"),		_T("¾"),		_T("⅕"),		_T("⅖"),		_T("⅗"),			// 0x7D51 - 0x7D58	93/49 - 93/56
			_T("⅘"),		_T("⅙"),		_T("⅚"),		_T("⅟₇"),		_T("⅛"),		_T("⅟₉"),		_T("⅟₁₀"),		_T("☀"),			// 0x7D59 - 0x7D60	93/57 - 93/64
			_T("☁"),		_T("☂"),		_T("⛄"),		_T("☖"),		_T("☗"),		_T("☖"),		_T("☗"),		_T("♦"),			// 0x7D61 - 0x7D68	93/65 - 93/72
			_T("♥"),		_T("♣"),		_T("♠"),		_T("⌺"),		_T("⦿"),		_T("‼"),		_T("⁉"),		_T("⛅"),		// 0x7D69 - 0x7D70	93/73 - 93/80
			_T("☔"),		_T("(大雨)"),		_T("☃"),		_T("(大雪)"),	_T("⚡"),		_T("(雷雨)"),	_T("　"),		_T("・"),			// 0x7D71 - 0x7D78	93/81 - 93/88
	...
	static const TSDCHAR *aszSymbolsTable5[] =
		{
			...
			_T("㟢"),		_T("庬"),		_T("弴"),		_T("彅"),		_T("德"),		_T("怗"),		_T("・"),		_T("愰"),			// 0x7541 - 0x7548	85/33 - 85/40
			_T("昤"),		_T("曈"),		_T("曙"),		_T("曺"),		_T("曻"),		_T("桒"),		_T("・"),		_T("椑"),			// 0x7549 - 0x7550	85/41 - 85/48
			_T("椻"),		_T("橅"),		_T("檑"),		_T("櫛"),		_T("𣏌"),		_T("𣏾"),		_T("𣗄"),		_T("毱"),			// 0x7551 - 0x7558	85/49 - 85/56
			_T("泠"),		_T("洮"),		_T("海"),		_T("涿"),		_T("淊"),		_T("淸"),		_T("渚"),		_T("潞"),			// 0x7559 - 0x7560	85/57 - 85/64
			_T("濹"),		_T("灤"),		_T("・"),		_T("𤋮"),		_T("煇"),		_T("燁"),		_T("爀"),		_T("玟"),			// 0x7561 - 0x7568	85/65 - 85/72
			_T("玨"),		_T("珉"),		_T("珖"),		_T("琛"),		_T("琡"),		_T("琢"),		_T("琦"),		_T("琪"),			// 0x7569 - 0x7570	85/73 - 85/80
	...
	static const TSDCHAR *aszSymbolsTable6[] =
		{
			...
			_T("藎"),		_T("蝕"),		_T("蟬"),		_T("蠋"),		_T("裵"),		_T("⻆"),		_T("諶"),		_T("跎"),			// 0x7631 - 0x7638	86/17 - 86/24
	...
	if((wCode >= 0x7A50U) && (wCode <= 0x7A74U)){
		tsd_strcpy(lpszDst, aszSymbolsTable1[wCode - 0x7A50U]);
	}
	else if((wCode >= 0x7C21U) && (wCode <= 0x7C7BU)){
		tsd_strcpy(lpszDst, aszSymbolsTable2[wCode - 0x7C21U]);
	}
	else if((wCode >= 0x7D21U) && (wCode <= 0x7D7BU)){
		tsd_strcpy(lpszDst, aszSymbolsTable3[wCode - 0x7D21U]);
	}
	else if((wCode >= 0x7E21U) && (wCode <= 0x7E7DU)){
		tsd_strcpy(lpszDst, aszSymbolsTable4[wCode - 0x7E21U]);
	}
	else if((wCode >= 0x7521U) && (wCode <= 0x757EU)){
		tsd_strcpy(lpszDst, aszSymbolsTable5[wCode - 0x7521U]);
	}
	else if((wCode >= 0x7621U) && (wCode <= 0x764BU)){
		tsd_strcpy(lpszDst, aszSymbolsTable6[wCode - 0x7621U]);
	}
	else{
